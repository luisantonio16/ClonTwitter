...
export const Seguidos = () => {
    //recogemos el id por el parametro
    ...
    const listaUsuarios = async (next = 1) => {
        ...
        const respuesta = await fetch(Global.url + "follow/siguiendo/" + params.id + "/" + next, {
            method: "GET",
            headers: {
        ...
        datosUsuarios.ususariosSeguidos.forEach(element => {
            limparArray = [...limparArray, element.seguido]
    ...
    return (
        <>
            <article className='usuarios-header'>
                <h2 className='usuarios-titulo'>Usuarios que sigue {usuario.nombre}</h2>
            </article>
